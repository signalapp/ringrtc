...
use crate::sim::sim_platform::{SimCallConnection, SimPlatform};
...
pub fn create_call_connection_factory() -> Result<SimCallConnectionFactory> {
    ...
    let ccf_ref =
        unsafe { ptr_as_arc_ptr(ccf_ptr as *mut CallConnectionFactory<SimPlatform>).unwrap() };
    info!(
        "create_call_connection_factory(): {:?}",
        ccf_ref.get_arc().lock().unwrap()
    );

    let ccf =
        unsafe { ptr_as_arc_mutex(ccf_ptr2 as *mut CallConnectionFactory<SimPlatform>).unwrap() };
...
pub fn create_call_connection(
    native_factory: &SimCallConnectionFactory,
    call_config: CallConfig,
    native_observer: Arc<Mutex<SimCallConnectionObserver>>,
) -> Result<Box<SimCallConnection>> {
