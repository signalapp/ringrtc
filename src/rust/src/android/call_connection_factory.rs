...
pub fn create_call_connection_factory(
    native_factory: *mut AppPeerConnectionFactory,
) -> Result<jlong> {
...
fn fetch_ice_servers<'a>(
    env: &'a JNIEnv,
    config: JObject,
) -> Result<Vec<PeerConnectionIceServer<'a>>> {
    // Get account manager object out of the config
    const ACCOUNT_MANAGER_FIELD: &str = "accountManager";
    const ACCOUNT_MANAGER_SIG: &str =
        "Lorg/whispersystems/signalservice/api/SignalServiceAccountManager;";
    let account_manager =
        jni_get_field(env, config, ACCOUNT_MANAGER_FIELD, ACCOUNT_MANAGER_SIG)?.l()?;

    const GET_TURN_SERVER_METHOD: &str = "getTurnServerInfo";
    const GET_TURN_SERVER_SIG: &str =
        "()Lorg/whispersystems/signalservice/api/messages/calls/TurnServerInfo;";
    let turn_server_info = jni_call_method(
        env,
        account_manager,
    ...
    ice_servers.push(PeerConnectionIceServer::new(
        env.new_string(STUN_SERVER)?,
        env.new_string("")?,
        env.new_string("")?,
    ));
...
fn create_java_ice_servers<'a>(
    env: &'a JNIEnv,
    ice_servers: &'a [PeerConnectionIceServer],
) -> Result<JList<'a, 'a>> {
    ...
    for server in ice_servers {
        ...
        const ICE_SERVER_CLASS_SIG: &str =
            "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V";
        let args = [
            JObject::from(server.uri).into(),
            JObject::from(server.username).into(),
            JObject::from(server.password).into(),
        ];
        let ice_server_obj = jni_new_object(env, ICE_SERVER_CLASS, ICE_SERVER_CLASS_SIG, &args)?;
...
pub fn create_call_connection(
    ...
    let call_id: CallId =
        jni_get_field(env, call_config, CALL_ID_FIELD, CALL_ID_SIG)?.j()? as CallId;
    ...
    let recipient: JObject =
        jni_get_field(env, call_config, RECIPIENT_FIELD, RECIPIENT_SIG)?.l()?;
    ...
    let out_bound: bool = jni_get_field(env, call_config, OUT_BOUND_FIELD, OUT_BOUND_SIG)?.z()?;
    ...
    jni_set_field(
        ...
        "iceServers",
        "Ljava/util/List;",
        JObject::from(java_ice_servers).into(),
    )?;
    ...
    let jni_owned_pc = unsafe {
        Java_org_webrtc_PeerConnectionFactory_nativeCreatePeerConnection(
            env.clone(),
            class,
            cc_factory.get_native_peer_connection_factory() as jlong,
            rtc_config,
            media_constraints,
