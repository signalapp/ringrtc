...
impl AndroidCallConnectionObserver {
    ...
    fn notify(&self, event: ClientEvent) -> Result<()> {
        ...
        let jitem = match env.call_static_method(
            class_object,
        ...
        let method_signature = format!(
            "(Lorg/signal/ringrtc/SignalMessageRecipient;JL{};)V",
            class_path
        );
        let args = [
            self.jrecipient.as_obj().into(),
            call_id_jlong.into(),
            jitem.into(),
        ];
    ...
    pub fn call_exception<'a>(&self, env: &'a JNIEnv, message: String) -> Result<JObject<'a>> {
        ...
        let args = [JObject::from(env.new_string(message)?).into()];

        static CALL_EXCEPTION_CLASS_SIG: &str = "(Ljava/lang/String;)V";
        let exception = match env.new_object(class_object, CALL_EXCEPTION_CLASS_SIG, &args) {
            Ok(v) => v,
            Err(_) => {
                return Err(AndroidError::JniCallConstructor(
                    class_path,
                    CALL_EXCEPTION_CLASS_SIG.to_string(),
                )
                .into())
            }
        };
    ...
    fn send_exception(&self, env: &JNIEnv, error: JObject) -> Result<()> {
        ...
        let method_signature: &str =
            "(Lorg/signal/ringrtc/SignalMessageRecipient;JLjava/lang/Exception;)V";
        let args = [
            self.jrecipient.as_obj().into(),
            call_id_jlong.into(),
            error.into(),
            ...
            Ok(android_error) => {
                match android_error {
                    ...
                    _ => {
                        ...
                        self.send_exception(
                            &env,
                            self.call_exception(&env, format!("{}", android_error))?,
                        )?
                    }
                }
            }
    ...
    fn on_add_stream(&self, stream: GlobalRef) -> Result<()> {
        ...
        let method_signature: &str =
            "(Lorg/signal/ringrtc/SignalMessageRecipient;JLorg/webrtc/MediaStream;)V";
        let args = [
            self.jrecipient.as_obj().into(),
            call_id_jlong.into(),
            stream.as_obj().into(),
