...
use jni::objects::{GlobalRef, JClass, JList, JObject, JString};
use jni::sys::{jboolean, jint, jlong, JNI_FALSE, JNI_TRUE};
use jni::{JNIEnv, JavaVM};

use crate::android::call_connection_observer::AndroidCallConnectionObserver;
use crate::android::error::{AndroidError, ServiceError};
...
use crate::core::util::{ptr_as_box, ptr_as_mut, ptr_as_ref};
...
impl CallPlatform for AndroidPlatform {
    ...
    fn app_send_offer(&self, call_id: CallId, offer: SessionDescriptionInterface) -> Result<()> {
        ...
        let result = jni_send_offer(
            &env,
            jcall_connection,
            self.recipient.as_obj(),
            call_id,
            offer,
        )?;
    ...
    fn app_send_ice_updates(&self, call_id: CallId, candidates: &[IceCandidate]) -> Result<()> {
        ...
        for candidate in candidates {
            ...
            let ice_update_message_obj = env.new_object(
                self.get_ice_update_message_class()?,
        ...
        let client_result = jni_send_ice_updates(
            &env,
            jcall_connection,
            self.recipient.as_obj(),
            ice_update_list,
...
impl AndroidPlatform {
    ...
    pub fn update(
        &mut self,
        jni_owned_pc: jlong,
        cc_observer: Box<AndroidCallConnectionObserver>,
    ) -> Result<()> {
        ...
        const ICE_UPDATE_MESSAGE_CLASS: &str =
            "org/whispersystems/signalservice/api/messages/calls/IceUpdateMessage";
        let ice_update_message_class = match env.find_class(ICE_UPDATE_MESSAGE_CLASS) {
            Ok(v) => v,
            Err(_) => {
                return Err(
                    AndroidError::JniClassLookup(String::from(ICE_UPDATE_MESSAGE_CLASS)).into(),
                )
            }
        };
        self.ice_update_message_class =
            Some(env.new_global_ref(JObject::from(ice_update_message_class))?);
    ...
    fn get_jcall_connection(&self) -> Result<JObject> {
        match self.jcall_connection.as_ref() {
            ...
            None => Err(RingRtcError::OptionValueNotSet(
                "get_jcall_connection()".to_string(),
                "jcall_connection".to_string(),
            )
            .into()),
        }
    }
    ...
    fn get_ice_update_message_class(&self) -> Result<JClass> {
        match self.ice_update_message_class.as_ref() {
            ...
            None => Err(RingRtcError::OptionValueNotSet(
                "get_ice_update_message_class()".to_string(),
                "ice_update_message_class".to_string(),
            )
            .into()),
        }
    }
...
pub fn native_get_native_peer_connection(
    ...
    if let Some(jni_owned_pc) = platform.jni_owned_pc.as_ref() {
        ...
        Err(RingRtcError::OptionValueNotSet(
            "native_get_native_peer_connection".to_string(),
            "jni_owned_pc".to_string(),
        )
        .into())
    }
}
...
fn jni_send_offer<'a>(
    ...
    const SEND_OFFER_MESSAGE_SIG: &str =
        "(Lorg/signal/ringrtc/SignalMessageRecipient;JLjava/lang/String;)Ljava/lang/Exception;";
    let args = [
        recipient.into(),
        call_id_jlong.into(),
...
pub fn native_validate_response_state(
    call_connection: *mut AndroidCallConnection,
) -> Result<jboolean> {
    let cc = unsafe { ptr_as_ref(call_connection)? };
...
fn jni_send_ice_updates<'a>(
    env: &'a JNIEnv,
    jcall_connection: JObject,
    recipient: JObject,
    ice_update_list: JList,
) -> Result<JObject<'a>> {
    info!("jni_send_ice_updates():");

    const SEND_ICE_UPDATE_MESSAGE_METHOD: &str = "sendSignalServiceIceUpdates";
    const SEND_ICE_UPDATE_MESSAGE_SIG: &str =
        "(Lorg/signal/ringrtc/SignalMessageRecipient;Ljava/util/List;)Ljava/lang/Exception;";
    let args = [recipient.into(), JObject::from(ice_update_list).into()];
    let result = jni_call_method(
        env,
        jcall_connection,
