...
use crate::webrtc::sdp_observer::{
    create_csd_observer, create_ssd_observer, SessionDescriptionInterface,
};
    ...
    T: CallPlatform,
{
    ...
    /// AppMediaStreams.
    stream_map: HashMap<*const RffiMediaStreamInterface, <T as CallPlatform>::AppMediaStream>,
}

// Send and Sync needed to share *const pointer types across threads.
unsafe impl<T> Send for WebRtcData<T> where T: CallPlatform {}

unsafe impl<T> Sync for WebRtcData<T> where T: CallPlatform {}
    ...
    T: CallPlatform,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "pc_interface: {:?}, data_channel: {:?}, data_channel_observer: {:?}",
            self.pc_interface, self.data_channel, self.data_channel_observer
        )
    }
}
    ...
    T: CallPlatform,
{
    ...
    pub fn send_pending_ice_updates(&self) -> Result<()> {
        ...
        if let Ok(mut ice_candidates) = self.pending_outbound_ice_candidates.lock() {
            ...
            info!(
                "send_pending_ice_updates(): Local ICE candidates length: {}",
                ice_candidates.len()
            );
            ...
            return Err(
                RingRtcError::MutexPoisoned("pending_outbound_ice_candidates".to_string()).into(),
            );
        }
    ...
    pub fn on_data_channel(
        ...
        call_connection: CallConnection<T>,
    ) -> Result<()> {
    ...
    pub fn notify_on_add_stream(&self, stream: MediaStream) -> Result<()> {
        ...
        if !webrtc.stream_map.contains_key(&media_stream_interface) {
            ...
            webrtc
                .stream_map
                .insert(media_stream_interface, app_media_stream);
        }
            ...
            while !*terminate_complete {
                terminate_complete = condvar.wait(terminate_complete).map_err(|_| {
                    RingRtcError::MutexPoisoned(
                        "CallConnection Terminate Condition Variable".to_string(),
                    )
                })?;
            }
        } else {
