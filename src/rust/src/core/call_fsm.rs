...
use crate::common::{CallDirection, CallId, CallState, Result};
...
impl fmt::Display for CallEvent {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let display = match self {
            ...
            CallEvent::RemoteVideoStatus(id, enabled) => format!(
                "RemoteVideoStatus, call_id: 0x{:x}, enabled: {}",
                id, enabled
            ),
            CallEvent::RemoteIceCandidate(_) => "RemoteIceCandidate".to_string(),
            CallEvent::LocalHangup => "LocalHangup".to_string(),
            CallEvent::LocalVideoStatus(enabled) => {
    ...
    T: CallPlatform,
{
    ...
    fn handle_event(
        ...
        match event {
            ...
            CallEvent::RemoteVideoStatus(id, enabled) => {
                self.handle_remote_video_status(cc, state, id, enabled)
            }
            CallEvent::RemoteIceCandidate(candidate) => {
                self.handle_remote_ice_candidate(cc, state, candidate)
            }
            CallEvent::LocalVideoStatus(enabled) => {
                self.handle_local_video_status(cc, state, enabled)
            }
            CallEvent::LocalIceCandidate(candidate) => {
                self.handle_local_ice_candidate(cc, state, candidate)
            }
            ...
            CallEvent::IceConnectionDisconnected => {
                self.handle_ice_connection_disconnected(cc, state)
            }
    ...
    fn handle_send_offer(&mut self, cc: CallConnection<T>, state: CallState) -> Result<()> {
    ...
    fn notify_client(&mut self, cc: CallConnection<T>, event: ClientEvent) {
        ...
        debug!(
            "fsm:notify_client(): spawning notify task, event: {}",
            event
        );
    ...
    fn handle_remote_hangup(
        ...
        match state {
            ...
            | CallState::CallConnected => self.notify_client(cc, ClientEvent::RemoteHangup),
    ...
    fn handle_remote_connected(
        ...
        match state {
            CallState::IceConnecting(_) | CallState::IceConnected => {
    ...
    fn handle_remote_ice_candidate(
        &mut self,
        cc: CallConnection<T>,
        state: CallState,
    ...
    fn handle_accept_call(&mut self, cc: CallConnection<T>, state: CallState) -> Result<()> {
        match state {
            CallState::IceConnecting(_) | CallState::IceReconnecting | CallState::IceConnected => {
    ...
    fn handle_local_hangup(&mut self, cc: CallConnection<T>, state: CallState) -> Result<()> {
        match state {
            CallState::Idle => self.unexpected_state(state, "LocalHangup"),
            _ => {
                let hang_up_future = lazy(move || cc.send_hang_up()).map_err(move |err| {
    ...
    fn handle_ice_connected(&mut self, cc: CallConnection<T>, state: CallState) -> Result<()> {
        match state {
            CallState::IceConnecting(_) => {
    ...
    fn handle_client_error(&mut self, cc: CallConnection<T>, error: failure::Error) -> Result<()> {
    ...
    fn handle_on_data_channel(
        ...
        match state {
            CallState::IceConnected | CallState::CallConnected => {
